...
def customers_list():
    ...
    try:
        ...
        from sqlalchemy import extract as sa_extract, select
        year_rows = (
        ...
        customers_with_distributions = (
            select(DistributionLogEntry.customer_id)
            .where(DistributionLogEntry.customer_id.isnot(None))
            .distinct()
        )
...
def customers_new_post():
    ...
    payload = {
        ...
        "sold_to_address1": request.form.get("sold_to_address1"),
        "sold_to_city": request.form.get("sold_to_city"),
        "sold_to_state": request.form.get("sold_to_state"),
        "sold_to_zip": request.form.get("sold_to_zip"),
